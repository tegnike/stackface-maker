...
まだ基準画像がない場合は、アプリの「標準表情生成」タブでキャラクター画像を選択し、「標準表情を生成...」を押します。すでに標準表情画像を用意できている場合は、この手順を飛ばして「4パターン作成」タブへ進んでください。アプリは `sample/standard/eyeON_mouthON.png` を第一リファレンス、選択したキャラクター画像を第二リファレンスとして画像生成します。

- 第一リファレンス: 構図、顔の大きさ、目口ONの状態、Stack-chan表示用の見え方
- 第二リファレンス: キャラクターデザイン、髪型、目、服装、色
- 生成結果がよければ「この画像を標準表情として使う」を押すと、そのまま通常の4パターン作成フローに進めます
- 必要に応じて「もっと顔アップ」「口を小さく」「構図を優先」「キャラ特徴を強める」で追加指示を入れて再生成できます

外部の画像生成ツールで作る場合や、追加指示を調整したい場合のプロンプト例:
...
1. **標準表情を選択**
   - 「4パターン作成」タブで、Stack-chanに表示したい基準表情を1枚選びます
   - 推奨作業サイズは640x480です
2. **感情画像を作成**
   - 基準画像の目/口状態をUIで指定します
